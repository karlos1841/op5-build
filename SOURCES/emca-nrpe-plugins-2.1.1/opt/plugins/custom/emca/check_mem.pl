...
sub tell_nagios {
    ...
    if ($opt_f) {
      ...
      if ($percent <= $opt_c) {
          finish("Memory usage CRITICAL: total:$total kB - used:$used - free:$free kB -  $percent% free!$perfdata",$exit_codes{'CRITICAL'});
      }
      elsif ($percent <= $opt_w) {
          finish("Memory usage WARNING: total:$total kB - used:$used - free:$free kB  - $percent% free!$perfdata",$exit_codes{'WARNING'});
      }
      else {
          finish("Memory usage OK: total:$total kB - used:$used - free:$free kB  - $percent% free.$perfdata",$exit_codes{'OK'});
      }
    }
    elsif ($opt_u) {
      my $percent = sprintf "%.1f", ($used / $total * 100);
      if ($percent >= $opt_c) {
          finish("Memory usage CRITICAL: total:$total kB - used:$used - free:$free kB  - $percent% used!$perfdata",$exit_codes{'CRITICAL'});
      }
      elsif ($percent >= $opt_w) {
          finish("Memory usage WARNING: total:$total kB - used:$used - free:$free kB  - $percent% used!$perfdata",$exit_codes{'WARNING'});
      }
      else {
          finish("Memory usage OK: total:$total kB - used:$used - free:$free kB  - $percent% used.$perfdata",$exit_codes{'OK'});
      }
    }
}
