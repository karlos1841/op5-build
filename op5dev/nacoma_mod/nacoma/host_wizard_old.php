...
function partition_scan($obj_id)
{
	...
	foreach($host->obj['hostgroups'] as $hostgroup)
	{
		...
		{
			if($hostgroup == "centrala_DMZ")
			{
				$connection = ssh2_connect('v-csb-p-op5dmz', 22, array('hostkey'=>'ssh-rsa'));
			}
			elseif($hostgroup == "centrala_KOZ")
			{
				$connection = ssh2_connect('v-csb-p-op5k1', 22, array('hostkey'=>'ssh-rsa'));
			}
			elseif($hostgroup == "centrala_POZ")
			{
				$connection = ssh2_connect('v-csb-p-op5p1', 22, array('hostkey'=>'ssh-rsa'));
			}
			elseif($hostgroup == "oddzial_BI")
			{
				$connection = ssh2_connect('vicsb-p-op5bi', 22, array('hostkey'=>'ssh-rsa'));
			}
			elseif($hostgroup == "oddzial_BY")
			{
				$connection = ssh2_connect('vbcsb-p-op5by', 22, array('hostkey'=>'ssh-rsa'));
			}
			elseif($hostgroup == "oddzial_GW")
			{
				$connection = ssh2_connect('vgcsb-p-op5gw', 22, array('hostkey'=>'ssh-rsa'));
			}
			elseif($hostgroup == "oddzial_SZ")
			{
				$connection = ssh2_connect('vscsb-p-op5sz', 22, array('hostkey'=>'ssh-rsa'));
			}
			elseif($hostgroup == "oddzial_ZG")
			{
				$connection = ssh2_connect('vzcsb-p-op5zg', 22, array('hostkey'=>'ssh-rsa'));
			}
			
		}
		if(ssh2_auth_pubkey_file($connection, 'monitor', '/var/www/html/.ssh/id_rsa.pub', '/var/www/html/.ssh/id_rsa'))
		{
                	foreach($hostgroups as $hostgroup)
                	{
                        	if($hostgroup == "z_template_Linux_ok")
                        	{
                                	$command = "/opt/plugins/custom/emca/bin/linux-scanner.pl -s https://10.119.240.140 -u api -p jioa891oo -o host -n {$host->obj['host_name']}";
                        	}
                        	elseif($hostgroup == "z_template_Windows_ok")
                        	{
                                	$win_command = "/opt/plugins/custom/emca/bin/windows-disk-scaner.pl -s https://10.119.240.140 -u api -p jioa891oo -o host -n {$host->obj['host_name']}";
                        	}
                	}
			...
			if(isset($command))
                	{
                        	...
                        	echo '<p><textarea readonly id="scan_output" rows="20" cols="120">';
				...
				echo stream_get_contents($stream_out);
                	...
                	elseif(isset($win_command))
                	{
				if(isset($_POST["partition"]))
                        	{
                        	        $win_command = $win_command . " --disk";
                        	}

				echo '<p><textarea readonly id="scan_output" rows="20" cols="120">';
				$stream = ssh2_exec($connection, $win_command);
				stream_set_blocking($stream, TRUE);
				$stream_out = ssh2_fetch_stream($stream, SSH2_STREAM_STDIO);
				echo stream_get_contents($stream_out);
				echo '</textarea></p>';
                	...
                	{
                        	echo "Cannot perform scan";
                	}
		}
		else
		{
			echo "Could not establish connection to host";
		}
        }
        $page->html_end();
}
...
elseif($_REQUEST['action'] === 'partition_scan') {
