...
function partition_scan($obj_id)
{
	...
	$windows_interface_scanner_path = "/opt/plugins/custom/emca/bin/windows_interface_scaner.pl";
	...
	if(isset($_POST["scan_hg"]))
	{
		...
		foreach($all_hosts as $host)
		{
			...
			if(preg_match('/I \(.*\) seem to be doing fine/', $match))
			{
				$out = shell_exec("{$run_as_user} {$run_ssh} {$windows_disk_scanner_path} -u {$username} -p {$password} -o host -n {$host} --disk;{$windows_interface_scanner_path} -u {$username} -P {$password} -p 1248 -m 5666 -T host -n {$host}");
				$output = $output . $out;
			}
			elseif(preg_match('/^NRPE .*/', $match))
			{
	...
	if(isset($_POST["scan"]))
	{
		...
		if(isset($_POST["windows"]))
		{
			...
			if(isset($_POST["network"]))
			{
				$command = $command . "{$windows_interface_scanner_path} -u {$username} -P {$password} -p 1248 -m 5666 -T host -n {$host->obj['host_name']};";
			}
		}
